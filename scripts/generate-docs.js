...
function parseModuleFile(filePath) {
  ...
  const sourceFileMatch = src.match(/source: WEB10 Source WebAPI (\S+)\)/);
...
for (const file of files) {
  ...
  const content = `# ${moduleName} — \`web10.${clientProp}\`

Source controller: [\`${sourceFile}\`](https://github.com/NextGenSoftwareUK/OASIS2/blob/main/WEB10/NextGenSoftware.OASIS.Web10.WebAPI/Controllers/${sourceFile})
...
const { isError, message, result } = await web10.${clientProp}.${exampleOp.jsName}(${exampleArgs});
...
const indexRows = moduleSummaries
  .map((m) => `| [\`web10.${m.clientProp}\`](modules/${m.moduleName}.md) | \`${m.routePrefix}\` | ${m.opCount} |`)
  .join('\n');

const totalOps = moduleSummaries.reduce((n, m) => n + m.opCount, 0);

const indexContent = `# WEB10 Singularity API — JavaScript SDK Reference

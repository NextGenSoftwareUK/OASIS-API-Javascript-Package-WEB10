...
```js
import { Web10Client } from 'https://esm.sh/@oasisomniverse/web10-api@1.0.3';
const client = new Web10Client({ baseUrl: 'https://api.web10.oasisomniverse.one' });
```
...
const client = new Web10Client({ baseUrl: '...', fetchImpl: fetch });
```
